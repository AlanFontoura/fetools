...
class VnfMigrationProcessor:
    ...
    def load_data(self):
        ...
        required_cols = [
            "Portfolio Firm Provided Key", "Date", "Value", "TWR_to_match",
            "FinTransfer", "OprTransfer", "Fees", "Expenses", "Household ID"
        ]
        
        for col in required_cols:
            if col not in df.columns:
                print(f"Warning: Column '{col}' not found in input. Filling with 0/Empty.")
        ...
        df.loc[df["Date"] == self.stitching_date, "Date"] = df.loc[df["Date"] == self.stitching_date, "Date"] - pd.Timedelta(days=1)
    ...
    def adjust_start_dates(self):
        ...
        min_dates = self.df.groupby("Portfolio Firm Provided Key")["Date"].transform("min")
        
        # Shift those dates back by 1 day
        mask = self.df["Date"] == min_dates
        self.df.loc[mask, "Date"] = self.df.loc[mask, "Date"] - pd.Timedelta(days=1)
    ...
    def apply_algebraic_plugs(self):
        ...
        df["MarketValuePrev"] = df.groupby("Portfolio Firm Provided Key")["Value"].shift(1)
        df["DatePrev"] = df.groupby("Portfolio Firm Provided Key")["Date"].shift(1)
        ...
        df["ManualReturn"] = np.where(
            df["MarketValuePrev"] != 0,
            df["PNL"] / df["MarketValuePrev"],
            0
        )
        ...
        is_alt = (df["Value"] <= df["MarketValuePrev"] * k_alt) & (df["Value"] > 0)
        ...
        if not plugs.empty:
            ...
            plugs_formatted = pd.DataFrame({
                "Portfolio Firm Provided Key": plugs["Portfolio Firm Provided Key"],
            ...
            self.df = self.df.sort_values(by=["Portfolio Firm Provided Key", "Date"])
            ...
            adjustment_map = plugs[["Portfolio Firm Provided Key", "Date", "CF_plug"]].copy()
            # Rename Date back to original to match the target row
            adjustment_map = adjustment_map.rename(columns={"CF_plug": "Offset_CF"})
            ...
            self.df = self.df.merge(adjustment_map, on=["Portfolio Firm Provided Key", "Date"], how="left")
            mask_adj = self.df["Offset_CF"].notna()
            # Subtract plug CF from OprTransfer (assuming Opr is the generic bucket)
            self.df.loc[mask_adj, "OprTransfer"] -= self.df.loc[mask_adj, "Offset_CF"]
    ...
    def triplicate_nodes(self, df_inputs: pd.DataFrame) -> pd.DataFrame:
        ...
        # Zero out metrics
        cols_to_zero = ["Value", "FinTransfer", "OprTransfer", "Fees", "Expenses"]
        main[cols_to_zero] = 0
    ...
    def generate_outputs(self):
        ...
        for i in range(0, len(households), self.config.batch_size):
            ...
            final_inputs["Portfolio Firm Provided Key"] = inputs_df["Portfolio Firm Provided Key"]
            final_inputs["Position Firm Provided Key"] = "history_instrument_USD"
            final_inputs["Date"] = inputs_df["Date"]
            final_inputs["Currency Split Type"] = inputs_df["Currency Split Type"]
            ...
            portfolios["Firm Provided Key"] = batch_df["Portfolio Firm Provided Key"].astype(str) + "_PrimarySleeve"
            ...
            bv["DateTradeAmt"] = 0 # Default for now as 'cash from trades' was not requested/implemented
            ...
            # Other 5.2 cols
            zero_cols = [
                "BookNumUnits", "BookValue", "DateTransferredCost", "DateRealizedPnl",
                "InternalBookNumUnits", "InternalBookValue", "DateInternalTransferredCost",
                "DateInternalRealizedPnl", "SettledBookValue", 
                "DateFinTransfAccrVal", "DateOprTransfAccrVal"
            ]
            for c in zero_cols:
                bv[c] = 0
